...
- **바텀시트 표준 동작(재검토 반영)**: 중복 핸들 제거(MethodSelect 자체 핸들·PayWindow 자체 라운드 삭제). BottomSheet에 딤 페이드 인/아웃·슬라이드다운 닫힘(280ms, 함수형 children으로 내부 닫기 버튼도 애니메이션 경유)·핸들 드래그 디스미스(80px 임계)·배경 스크롤 락·ESC 닫기. **모달 시트는 앱 컬럼(430px)을 넘지 않게 폭 제한**(딤은 풀블리드, 시트만 centering).
- **이동 탭 재기획(사용자 지시)**: 페이지 = **전면 지도**(추천/검색 스팟 마커 — 마커 탭으로도 목적지 선택, 시트 덮임을 감안해 center 남측 보정). 바텀시트는 페이지 위 **오버레이(`OverlaySheet`)**: 드래그로 내리면 닫히는 대신 **peek(핸들+타이틀 62px)으로 접히고** 지도·스텝은 유지, peek 탭/위 스와이프로 재출현. 단계 변경 시 자동 재펼침, 호출 진행 중 접기 잠금. 스텝 뒤로가기는 플로팅 백 버튼 전담(기존 InlineSheet의 드래그=뒤로가기 폐기). 페이지 상단 Header 제거(시트 타이틀로 대체).

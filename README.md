...
- **montage-mcp-server** — 원티드 Montage 컴포넌트·토큰·아이콘 조회. **npx 로컬 stdio**(`@wanteddev/wds-mcp`, 공식 방식) — 공개 문서를 페치해 서빙하므로 Wanted 로그인 불필요, GitHub Packages 토큰만(전역 `~/.npmrc`). *(호스티드 `montage.wanted.co.kr/mcp`는 Wanted OAuth 게이트라 미사용)*
